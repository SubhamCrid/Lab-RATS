...
check_requirements() {
    ...
    if ! command -v java &> /dev/null; then
        ...
        echo ""
        read -p "    Choose option: " JAVA_OPTION
...
configure_logo() {
    ...
    echo ""
    read -p "    Choose option [3]: " LOGO_OPTION
    LOGO_OPTION=${LOGO_OPTION:-3}
    ...
    if [ -n "$LOGO_PATH" ]; then
        ...
        if [ "$LOGO_OPTION" != "3" ] && [ -d "$RES_DIR/mipmap-anydpi-v26" ]; then
...
build_apk() {
    ...
    if [ -n "$JAVA_HOME" ]; then
        echo -e "${GREEN}[✓] Using Java at: $JAVA_HOME${NC}"
    fi
...
main_menu() {
    ...
    echo "    6. Exit"
    echo ""
    read -p "    Choose option: " MENU_OPTION
